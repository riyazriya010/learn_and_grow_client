...
const AdminDashboard = () => {
    ...
    const pieData = {
        labels: courseSales.length > 0 ? courseSales.map(item => item.categoryName) : ["No Sales"],
        datasets: [{
        ...
        <>
            <div className="flex flex-col min-h-screen bg-white">
                ...
                <div className="min-h-screen p-6 bg-white-100">
                    ...
                    {/* Active User and Mentors */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <Card title="Active Students" value={`${data?.getUsers || 0}`} className="bg-blue-50" />
                        <Card title="Active Mentors" value={`${data?.getMentors || 0}`} className="bg-gray-50" />
                    </div>

                    {/* Course Performance */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                        <Card title="Total Courses" value={data?.totalCourses || 0} className="bg-blue-50" />
                        <Card title="Most Enrolled Category" value={`${data?.mostEnrolledCourse?.categoryName || "N/A"} / ${data?.mostEnrolledCourse?.enrollments || "0"}`} className="bg-gray-50" />
                        <Card title="Least Enrolled Category" value={`${data?.leastEnrolledCourse?.categoryName || "N/A"} / ${data?.leastEnrolledCourse?.enrollments || "0"}`} className="bg-blue-50" />
                    </div>
